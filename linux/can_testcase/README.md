...
├──────────┼────────┼────────┼────────┼────────┼────────┼────────┼────────┼────────┤
│ total    │ T4-T0  │ exact  │    142 │    171 │    219 │    240 │    263 │    175 │
│ A tx     │ T1-T0  │ approx │    ... │        │        │        │        │        │
│ B mcu    │ T2-T1  │ exact  │      2 │      4 │      7 │        │        │        │
│ C uplink │ T3-T2  │ approx │    ... │        │        │        │        │        │
│ D rx     │ T4-T3  │ exact  │    ... │        │        │        │        │        │
├──────────┴────────┴────────┴────────┴────────┴────────┴────────┴────────┴────────┤
...
trust 列:`exact` = 单时钟无映射误差;`approx` = 含网关钟映射偏差(A 偏大
多少 C 就偏小多少,total/B/D 不受影响);映射偏差为负被钳零的样本会在表尾
note 行计数。

`--csv` 生成 `pair-A-B.csv`(seq + 五时刻原始 ns),可直接 Excel/脚本分析。
...
| 步骤 | 命令 | 看什么 |
|---|---|---|
| 1. 时延基线 | `./canperf --count 100000` | 五段直方图;B 段稳定;counters delta 全 0 |
...
- **B 段**为个位数 µs 且恒正 → 时钟映射健康;B 异常大/集中为 0 → 映射
  偏差大,A/C 数值仅供参考,总时延仍可信。
- 总时延 ≈ A+B+C+D(µs 级闭合);明显不闭合说明某段时刻缺失(看
  "网关时戳缺失"计数)。
- C 段天然含 0~50µs 聚合等待(协议 T_agg),其 p50 比 A 段高是正常的。
